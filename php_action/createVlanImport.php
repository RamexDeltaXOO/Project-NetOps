			...
			if(move_uploaded_file($_FILES['brandfile']['tmp_name'], $url)) {
				...
				if(!empty($no_error_data)) {
					foreach ($no_error_data as $value) {
						$select_sql = "SELECT * FROM vlan_list WHERE vlan_name ='$value[0]'";
						$result = $connect->query($select_sql);
						if($result->num_rows == 0) { 
						$sql = "INSERT INTO vlan_list (vlan_name, vlan_ip, vlan_active, vlan_status) VALUES ('$value[0]', '$value[1]', '$value[2]', '$value[2]')";
							if($connect->query($sql) === TRUE) {
								$valid['success'] = true;
								$valid['messages'] = "Successfully Added";
							} else {
								$valid['success'] = false;
								$valid['messages'] = "Error while adding the vlan";
							}
						} else {
							$valid['success'] = true;
							$valid['messages'] = "Successfully Added";
