...
$(document).ready(function() {
	...
	$("#submitBrandForm").unbind('submit').bind('submit', function() {
		...
		var brandIp = $("#brandIp").val();
		var brandStatus = $("#brandStatus").val();

		if(brandName == "") {
			$("#brandName").after('<p class="text-danger">Vlan Name field is required</p>');
		...
		if(brandIp == "") {
			$("#brandIp").after('<p class="text-danger">Vlan Ip field is required</p>');
			$('#brandIp').closest('.form-group').addClass('has-error');
		} else {
			// remov error text field
			$("#brandIp").find('.text-danger').remove();
			// success out for form 
			$("#brandIp").closest('.form-group').addClass('has-success');	  	
		}
		...
		if(brandName && brandIp && brandStatus) {
			var form = $(this);
			// button loading
...
function editBrands(brandId = null) {
	if(brandId) {
		...
		$.ajax({
			url: 'php_action/fetchSelectedVlan.php',
			type: 'post',
			...
			success:function(response) {
				...
				$('#editBrandName').val(response.vlan_name);
				
				$('#editBrandIp').val(response.vlan_ip);
				// setting the brand status value
				$('#editBrandStatus').val(response.vlan_active);
				// brand id 
